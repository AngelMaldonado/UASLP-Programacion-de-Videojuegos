...
std::vector<GLfloat> IndexedModel::GetDataVector()
{
    // Vector para todos los datos
    std::vector<float> data;

    // Recorre los datos de las posiciones
    for(unsigned int i = 0; i < positions.size(); i++)
    {
        // Agrega las coordenadas del vertice
        data.push_back(positions[i].x);
        data.push_back(positions[i].y);
        data.push_back(positions[i].z);

        // Agrega las coordendas de textura correspondientes
        if(texCoords.size() > 0)
        {
            data.push_back(texCoords[indices[i]].x);
            data.push_back(texCoords[indices[i]].y);
        }

        // Agrega las coordenadas de normales correspondientes
        if(normals.size() > 0)
        {
            data.push_back(normals[indices[i]].x);
            data.push_back(normals[indices[i]].y);
            data.push_back(normals[indices[i]].z);
        }

        // printf("%f, %f, %f\n", positions[indices[i]].x, positions[indices[i]].y, positions[indices[i]].z);

        // // Agrega las coordenas de textura
        // if(texCoords.size() > 0)
        // {
        //     data.push_back(texCoords[indices[i]].x);
        //     data.push_back(texCoords[indices[i]].y);
        ...
        // printf("%f, %f\n", texCoords[indices[i]].x, texCoords[indices[i]].y);

        // // Agrega las coordenadas de normales
        // if(normals.size() > 0)
        // {
        //     data.push_back(normals[indices[i]].x);
        //     data.push_back(normals[indices[i]].y);
        ...
        // printf("%f, %f, %f\n", normals[indices[i]].x, normals[indices[i]].y, normals[indices[i]].z);
    }
